/*********************************************************************/
/******          db_margin.cpp - M.A. Acero O. - 06-12-2007        ******/
...
int main(int argc, char* argv[])
{
  ...
  // path to read/write data
    string path1 = argv[3];
    string chi_file = "chi2_s2t-eps_surface_RATE.txt";
    string min_chi = "chi2_minumum_RATE.txt";
    //string chi_file = "chi2_s2t-eps_surface_SPEC.txt";
    //string min_chi = "chi2_minumum_SPEC.txt";
...
//    const int row = (sal+1)*(mal+1);
    ...
    double minchi = c_min[0];
  
    for(int l=0;l<row;l++)             //Chi2 data
        for(int m=0;m<COL;m++)
            min1 >> matrix[l][m];

    string s2t_file = "db_s2t_chi2_RATE.txt";
    string dmn_file = "db_eps_chi2_RATE.txt";
