...
int MINOS_EnergyS_ntuple()
{
    ...
    TFile *fntuple = new TFile("./minos_ntuple.root","READ");
...
//    T->Draw(Form("Ep >> nosc_spect_hist_%d",iAD),cutES,"");
    ...
    for (int ee = 0 ; ee < de ; ee++) {
        ...
        nosc_spect_hist_diff[ee]->GetYaxis()->SetTitle("Events(Original - Corrected)");
        for (int i = 1 ; i <= NB_numu110 ; i++) {
            double binCont = nosc_spect_hist->GetBinContent(i) - nosc_spect_hist_e[ee]->GetBinContent(i);
    ...
    TFile *fEscale = new TFile("./minos_EScaleDerivative.root","recreate");
    fFit4->Write();
    fFit7->Write();
