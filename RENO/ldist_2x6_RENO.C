...
{ // begin
  ...
  for (int ir=0; ir<nRea; ir++){
    ...
    int k = 0*nRea+ir;
    double wgtn = massesDet[0]*th_pow[ir]/baselines[0][ir]**2;
    histo_ldist_RENO_near->SetBinContent(k+1,wgtn);
    //cout << " wgtn = " << wgtn << " k = " << k <<endl;
    
    int kk = 0*nRea+ir;
    double wgtf = massesDet[1]*th_pow[ir]/baselines[1][ir]**2;
  ...
  TFile *fout = new TFile("files/ldist_RENO_2x6.root","recreate");
  fout->cd();
  ...
  for (int i=0;i<Nevt;i++){
    ...
    int bl_idx_near = (int*)histo_ldist_RENO_near->GetRandom();
    int bl_idx_far = (int*)histo_ldist_RENO_far->GetRandom();
  ...
  TFile *fout = new TFile("files/ldist_RENO_gen.root","recreate");
  fout->cd();
