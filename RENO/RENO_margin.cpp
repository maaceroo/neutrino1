/********************************************************************************/
/******         RENO_margin.cpp - M.A. Acero O. D. J. Polo - 02-02-2018    ******/
/***** Program to get the marginalization analysis for the oscillation   ********/
/***** parameters. It works for the anylisis of RENO data.               ********/
...
int main(int argc, char* argv[])
{
    ...
    {
        cout << "use:  " << endl;
        cout <<  argv[0] << " s2T(int) epsN(int) path_to_data_dir" << endl;
        exit(1);
    }
    // path to read/write data
    string path1 = argv[3];
    string chi_file = "files/chi2_s2t-a_surface_RATE.txt";
    string min_chi =  "files/chi2_minimum_RATE.txt";
    ...
    for(int l=0;l<row;l++)             //Chi2 data
        for(int m=0;m<COL;m++)
            min1 >> matrix[l][m];
    cout << matrix[10][0] << " " << matrix[10][1] << " " << matrix[10][2] << endl;

    string s2t_file = "files/RENO_s2t_chi2_RATE.txt";
    string dmn_file = "files/RENO_a_chi2_RATE.txt";
